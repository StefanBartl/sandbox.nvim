---@module 'sandbox.adapters.nerdctl.networks.prune_networks'
--- Nerdctl Adapter: Function to prune unused networks

local run_argv = require("sandbox.util.run_argv")

local M = {}

--- Remove all unused local networks without blocking the UI thread.
--- @param on_done? fun(ok: boolean, err: string|nil)
--- @return table handle with a `:stop()` method
function M.prune_networks(on_done)
  return run_argv.run_async_captured({ "nerdctl", "network", "prune", "-f" }, function(ok, output, code)
    if not on_done then
      return
    end
    if ok then
