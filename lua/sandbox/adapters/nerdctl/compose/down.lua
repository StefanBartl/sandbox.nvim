---@module 'sandbox.adapters.nerdctl.compose.down'
--- Nerdctl Adapter: Stop and remove a compose project
...
function M.down(file, on_done)
  return run_argv.run_async_captured({ "nerdctl", "compose", "-f", file, "down" }, function(ok, output, code)
    if not on_done then
      return
    end
    if ok then
