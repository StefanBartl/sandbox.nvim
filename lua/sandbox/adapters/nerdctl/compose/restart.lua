---@module 'sandbox.adapters.nerdctl.compose.restart'
--- Nerdctl Adapter: Restart a compose project
...
function M.restart(file, on_done)
  return run_argv.run_async_captured({ "nerdctl", "compose", "-f", file, "restart" }, function(ok, output, code)
    if not on_done then
      return
    end
    if ok then
