...
`lua/containers/@types/init.lua` wurde ergänzt (`Containers.Engine`, `Containers.Config.Options`).
...
(siehe `lua/containers/bindings/usrcmds/container_commands_buffer.lua`).
...
- **Which-Key**: Nicht anwendbar — das Plugin definiert keine Keymaps, nur Usrcmds.
  Falls künftig Keymaps sinnvoll werden, gehören sie nach `lua/containers/bindings/keymaps/`
...
- **lib.nvim-Migration**: `containers.notify` (optionaler Wrapper um `lib.nvim.nvim.notify`,
  ...
  Adaptern/Usecases auf `containers.notify` wäre ein separater, rein mechanischer Task
...
- **lib.nvim.nvim.usercmd**: Könnte den `pcall`+`vim.notify`-Boilerplate in
  `lua/containers/bindings/usrcmds/*.lua` ersetzen. Ebenfalls als separater Folge-Task
  vorgemerkt.
